...
public class DBBackendVvEntity {
	...
	public static int delete(OpenHelper openHelper, Uri uri, String selection, String[] selectionArgs) {
		...
		case ENTITY:
			count = db.delete(DB.VvEntity.TABLE_NAME, selection, selectionArgs);
			break;

		case ENTITY_ID:
			String id = uri.getPathSegments().get(1);
			count = db.delete(DB.VvEntity.TABLE_NAME, DB.NAME_ID + "=" + id + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""),
					selectionArgs);
			break;
	...
	public static String getType(Uri uri) {
		switch (sUriMatcher.match(uri)) {
		case ENTITY:
			return DB.VvEntity.CONTENT_TYPE;

		case ENTITY_ID:
			return DB.VvEntity.CONTENT_ITEM_TYPE;
	...
	public static Cursor query(OpenHelper openHelper, Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		qb.setTables(DB.VvEntity.TABLE_NAME);
		...
		if (TextUtils.isEmpty(sortOrder)) {
			orderBy = DB.VvEntity.SORTORDER_DEFAULT;
		} else {
			orderBy = sortOrder;
		}
	...
	public static int update(OpenHelper openHelper, Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		switch (sUriMatcher.match(uri)) {
		case ENTITY:
			count = db.update(DB.VvEntity.TABLE_NAME, values, selection, selectionArgs);
			break;

		case ENTITY_ID:
			String id = uri.getPathSegments().get(1);
			count = db.update(DB.VvEntity.TABLE_NAME, values, DB.NAME_ID + "=" + id + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""),
					selectionArgs);
			break;
	...
	public static Uri insert(OpenHelper openHelper, Uri uri, ContentValues initialValues) {
		...
		SQLiteDatabase db = openHelper.getWritableDatabase();
		long rowId = db.insert(DB.VvEntity.TABLE_NAME, null, values);
		if (rowId > 0) {
			Uri retUri = ContentUris.withAppendedId(VvEntity.CONTENT_URI, rowId);
			return retUri;
		}
	...
	static {
		...
		sUriMatcher.addURI(VvContentProvider.AUTHORITY, DB.VvEntity.CONTENT_ITEM_NAME, ENTITY);
		sUriMatcher.addURI(VvContentProvider.AUTHORITY, DB.VvEntity.CONTENT_ITEM_NAME + "/#", ENTITY_ID);
